...
## Замена html спец. символов в тексте
def unescape(s):
    t = s
    d = {'&amp;': '&',
         '&reg;': "",
...
## Функция возвращает список всех .eml в выбранной директории
def get_all_eml_files_in_dir(directory_):
    farray = os.listdir(directory_)
...
##Функця получает список аргументов, и возвращает строку для csv форматирования по их количеству
def get_str_to_format(list_args):
    s = ''
    l = len(list_args)
    for i in range(l):
        s += '{' + str(i) + '};'
...
##Функция замены сивола евро (UTF8) в строке на пробел.
def replace_euro_char(s):
    res = s
    if '\\\\u0432\\\\u201a' in s:
        idx = s.index('\\\\u0432\\\\u201a')
