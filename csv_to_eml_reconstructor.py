...
def get_purchase_information_tax_row(dict_names, row_dict,
                                     position):  ##возвращает строку нижней таблицы раздела Purchase Information (html-код)
    size = 1
...
def get_purchase_information_pcakage_row(dict_names, row_dict,
                                         position):  ##возвращает строку верхней таблицы раздела Purchase Information (html-код)
    ...
    row_html = """<tr>
				    ...
				    </tr>""".format(param_name=param_name,
                                    param_value=param_value)  ##Форматирует html исходя из значения и заголовка текущей позиции
    return row_html
...
def get_purchase_information(row_dict):  # возвращает html-код сформированного раздела Purchase Information
    ...
    result = result.format(package_name=package_name, term_of_lease=term_of_lease,
                           package_price_paid=row_dict['package_price_paid'], setup_price=setup_price)
...
def get_contact_information_row(dict_names, row_dict, position):
    ...
    try:
        ...
        if param_name == 'Address':
            ...
            return '''{param_name}: <a href=mailto:{param_value}>{param_value}</a><br /> '''.format(
...
def get_contact_information_html(row_dict):  ##Возвращает сформированный раздел Contact Information(html-код)
    ...
    # Возвращает строку, по формату подходящую для разделов Account Information и Billing Information


def get_account_or_billing_information_row(dict_names, row_dict, position):
    ...
    if position == 'account_url':  ##исключение для position == account_url - другой формат возвращаемой html строки
        return '{position_name}: <A href={position_value}>{position_value}</a><br /> '.format(
            position_name=position_name, position_value=position_value)
    else:
        return '{position_name}: {position_value}<br /> '.format(position_name=position_name,
                                                                 position_value=position_value)  ##Форматирование строти исходя из заголовка и значения
...
def eml_write(row_dict, row_index, out_path):  ##Формирует eml исходя из полученной из csv строки
    ...
    msg['From'] = sender
    msg['To'] = recepiant
    contact_information = get_contact_information_html(
        row_dict)  ##Получить сформированный html раздела Contact Information
    purchase_information = get_purchase_information(
        row_dict)  ##Получить сформированный html раздела Purchase Information
    ...
    outfile = open(out_path + "\\{0:0>8}.eml".format(row_index),
                   'w')  ##Открытие файла на запись - имя файла зависит от текущей строки
        ...
        if is_first:  ##Если строка - первая
            ...
            try:
                if d['setup_price'] != '':  ##Если SetupPrice != ''
                    sub_total = float(d['package_price_paid']) + float(
                        d['setup_price'])  ##SubTotal = setup_price+package_price_paid
